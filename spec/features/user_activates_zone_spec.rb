...
RSpec.feature "Zone activation", type: :feature do
  ...
  scenario "registered toggles a zone with incorrect data", js: true do
    VCR.use_cassette "log in with devices" do
      make_test_user
      login("rachiobeta", "test1234")
      ...
      within('.zone-1') do
        ...
        expect(page).to have_content("Sorry, please enter a value between 0 and 10800 seconds")
        ...
        expect(page).to have_content("Ready to run!")
        expect(page).to_not have_content("Sorry, please enter a value between 0 and 10800 seconds")
      end
    end
  end
